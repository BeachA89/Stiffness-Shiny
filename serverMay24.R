...
server <- function(input, output) {
  ...
  output$resultstableLeft1.7 <- renderTable({
    ...
    for (w in 1:q){
      ...
      EccStiffHop[w] = ((max(Hop) - BW)/(HopDisp[ContactHop] - min(HopDisp)))/Mass
    ...
    values$Left1.7Averages <-  cbind(avgEccCOMDisp, avgContactTime, avgFlightTime, avgEccStiffness, avgPeakForceBW, avgJumpFreq)
